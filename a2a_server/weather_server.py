...
注意：
    langchain agent的tool实现了 调用MCP server:
    MCP server -> to_structured_langchain_tool -> langchain tool -> create_tool_calling_agent -> A2AServer
"""
...
from utils.mcp_tools import to_structured_langchain_tool  # 修复版工具转换函数, 修复python_a2a的bug
...
# 缓存 MCP 工具，避免每次请求都发 HTTP GET 获取 Schema
_cached_tools = None
...
agent_card = AgentCard(
    ...
    skills=[  # 技能列表：描述本代理具体能做什么
        AgentSkill(
            ...
            examples=["北京 2025-07-30 天气", "上海未来5天", "今天天气如何", "广州明天的天气怎样"]  # 使用示例
        )
    ]
)
...
class WeatherServer(A2AServer):
    ...
    def __init__(self):
        ...
        调用父类的 __init__.py 注册 Agent Card。
        """
