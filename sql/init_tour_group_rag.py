...
检索向量库流程/RAG流程：
1. 离线存储阶段：知识库数据存入milvus向量库中，嵌入模型使用阿里云百炼的text-embedding-v3模型，使用API调用
2. 在线问答阶段：
    1. 用户输入问题或查询query
    2. 使用 Qwen Embedding API（text-embedding-v3） 生成query向量
    3. 在 Milvus 中进行向量检索（IVF_FLAT + COSINE），找到最相似的旅游团数据, 得到检索上下文
    4. 将用户问题query + 检索上下文 + 对话历史 拼接为prompt，传入LLM，生成答案
"""
